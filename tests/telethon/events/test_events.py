...
class TestInitMissingCoverage:
    ...
    def test_unregister_with_invalid_type(self):
        @events.register(events.NewMessage)
        async def handler(event):
            pass
        # This should still remove entries even if type doesn't match exactly
        result = events.unregister(handler, 'some_type')
        assert result > 0


class TestCommonMissingCoverage:
    @pytest.mark.asyncio
    async def test_into_id_set_with_string_input(self):
        from telethon.events.common import _into_id_set
        client = TelegramClient(None, 1, '1')
        
        user = types.User(id=123, access_hash=456, username='testuser')
        client.get_input_entity = AsyncMock(return_value=user)
        
        result = await _into_id_set(client, 'testuser')
        assert len(result) > 0

    @pytest.mark.asyncio
    async def test_event_builder_filter_resolved_false(self):
        builder = events.NewMessage(chats={123})
        # Don't mark as resolved
        assert builder.filter(MagicMock()) is None

    def test_event_common_stringify(self):
        event = events.EventCommon()
        result = event.stringify()
...
class TestUserUpdateMissingCoverage:
    def test_user_update_build_none_return(self):
        update = types.UpdateUserStatus(user_id=123, status=None)
        event = events.UserUpdate.build(update)
        assert event is None
...
class TestInlineQueryMissingCoverage:
    def test_inline_query_filter_returns_super_result(self):
        builder = events.InlineQuery(func=lambda e: e.text_len > 0)
        event = MagicMock()
        event.text = 'test query'
...
class TestAlbumMissingCoverage:
    @pytest.mark.asyncio
    async def test_album_event_edit_no_caption(self):
        from telethon.events.album import Album
        
        message1 = types.Message(id=1, peer_id=types.PeerUser(123), date=None, message='', out=False, grouped_id=999)
        message2 = types.Message(id=2, peer_id=types.PeerUser(123), date=None, message='', out=False, grouped_id=999)
        
        event = Album.Event([message1, message2])
        event._client = MagicMock()
        event._client.edit_message = AsyncMock(return_value='result')
        
        # Edit when no messages have text should edit first message
        result = await event.edit('new text')
        assert result == 'result'
        # Should edit the first message
        event._client.edit_message.assert_called_once()

class TestAlbumExtendedCoverage:
    @pytest.mark.asyncio
    async def test_album_event_respond_with_args(self):
        from telethon.events.album import Album
        ...
        result = await event.respond('response', parse_mode='html')
        assert result == 'result'

    @pytest.mark.asyncio
    async def test_album_event_reply_with_args(self):
        from telethon.events.album import Album
        ...
        result = await event.reply('reply', parse_mode='html')
        assert result == 'result'

    @pytest.mark.asyncio
    async def test_album_event_forward_to_with_args(self):
        from telethon.events.album import Album
        ...
        result = await event.forward_to('dest', as_album=True)
        assert result == 'result'

    @pytest.mark.asyncio
    async def test_album_event_edit_with_link_preview(self):
        from telethon.events.album import Album
        ...
        event._client = MagicMock()
        event._client.edit_message = AsyncMock(return_value='result')
        event.messages[0].web_preview = None
        
        result = await event.edit('new text')
        assert result == 'result'

    @pytest.mark.asyncio
    async def test_album_event_edit_with_buttons(self):
        from telethon.events.album import Album
        ...
        event._client = MagicMock()
        event._client.edit_message = AsyncMock(return_value='result')
        event.messages[0].reply_markup = None
        
        result = await event.edit('new text')
        assert result == 'result'

    @pytest.mark.asyncio
    async def test_album_event_delete_with_args(self):
        from telethon.events.album import Album
        ...
        result = await event.delete(revoke=True)
        assert result == 'result'

    @pytest.mark.asyncio
    async def test_album_event_mark_read_without_client(self):
        from telethon.events.album import Album
    ...
    @pytest.mark.asyncio
    async def test_album_event_pin_without_notify(self):
        from telethon.events.album import Album
        ...
        event = Album.Event([message1, message2])
        event._client = MagicMock()
        event._client.pin_message = AsyncMock(return_value='result')
        
        result = await event.pin(notify=False)
        assert result == 'result'


class TestCallbackQueryExtendedCoverage:
    @pytest.mark.asyncio
    async def test_callback_query_event_answer_all_params(self):
        query = types.UpdateBotCallbackQuery(
            query_id=1,
            user_id=123,
            peer=types.PeerUser(456),
    ...
    @pytest.mark.asyncio
    async def test_callback_query_event_respond_with_args(self):
        query = types.UpdateBotCallbackQuery(
            query_id=1,
            user_id=123,
            peer=types.PeerUser(456),
    ...
    @pytest.mark.asyncio
    async def test_callback_query_event_reply_with_args(self):
        query = types.UpdateBotCallbackQuery(
            query_id=1,
            user_id=123,
            peer=types.PeerUser(456),
    ...
    @pytest.mark.asyncio
    async def test_callback_query_event_edit_all_params(self):
        msg_id = types.InputBotInlineMessageID(id=123, dc_id=1, access_hash=456)
        query = types.UpdateInlineBotCallbackQuery(
            query_id=1,
            user_id=123,
        ...
        result = await event.edit('new text', parse_mode='html', link_preview=True)
        assert result == 'result'

    @pytest.mark.asyncio
    async def test_callback_query_event_delete_with_args(self):
        query = types.UpdateBotCallbackQuery(
            query_id=1,
            user_id=123,
            peer=types.PeerUser(456),
...
class TestUserUpdateExtendedCoverage:
    def test_user_update_property_empty_action(self):
        event = events.UserUpdate.Event(peer=types.PeerUser(123), action=None)
        assert event.typing is None
        assert event.uploading is None
        ...
        assert event.sticker is None
        assert event.photo is None

    def test_user_update_property_empty_status(self):
        event = events.UserUpdate.Event(peer=types.PeerUser(123), status=None)
        assert event.last_seen is None
        assert event.until is None
        assert event.online is False
        assert event.recently is False
        assert event.within_weeks is False
        assert event.within_months is False


class TestCommonExtendedCoverage:
    @pytest.mark.asyncio
    async def test_event_builder_filter_blacklist_no_match(self):
        builder = events.NewMessage(chats={123}, blacklist_chats=True)
        builder.resolved = True
        event = MagicMock()
        event.chat_id = 456
    ...
    @pytest.mark.asyncio
    async def test_event_builder_filter_returns_func_result(self):
        builder = events.NewMessage(func=lambda e: e.value == 'custom')
        builder.resolved = True
        event = MagicMock()
        event.message.out = False
        event.message.fwd_from = None
    ...
    def test_event_common_to_dict_with_multiple_attrs(self):
        event = events.EventCommon()
        event.attr1 = 'value1'
        event.attr2 = 'value2'
        ...
        assert 'attr1' in d
        assert 'attr2' in d
        assert d['attr1'] == 'value1'
        assert d['attr2'] == 'value2'

    def test_event_common_str_with_complex_dict(self):
        event = events.EventCommon()
        event.nested = {'key': 'value'}
        event.list_attr = [1, 2, 3]
        
        result = str(event)
        assert 'nested' in result or 'list_attr' in result
