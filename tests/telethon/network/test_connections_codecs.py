...
class TestHttpPacketCodec:
    ...
    @pytest.mark.asyncio
    async def test_http_read_packet(self, mock_loggers):
        """Test reading HTTP packet."""
        conn = ConnectionHttp('127.0.0.1', 443, 1, loggers=mock_loggers)
        codec = HttpPacketCodec(conn)

        # Mock HTTP response - Content-Length should be found
        class MockReader:
            def __init__(self):
                self.call_count = 0
    ...
    @pytest.mark.asyncio
    async def test_http_read_packet_zero_length(self, mock_loggers):
        """Test reading HTTP packet with zero length."""
        conn = ConnectionHttp('127.0.0.1', 8080, 1, loggers=mock_loggers)
        codec = HttpPacketCodec(conn)

        # Mock HTTP response with zero length
        class MockReader:
            def __init__(self):
                self.call_count = 0
    ...
    @pytest.mark.asyncio
    async def test_http_read_packet_large_length(self, mock_loggers):
        """Test reading HTTP packet with large length."""
        conn = ConnectionHttp('127.0.0.1', 8080, 1, loggers=mock_loggers)
        codec = HttpPacketCodec(conn)

        # Mock HTTP response with large length
        large_length = 10000
        ...
        class MockReader:
            ...
            async def readline(self):
                ...
                if self.call_count == 1:
                    ...
                    return f'Content-Length: {large_length}\r\n'.encode()
                return b'\r\n'

            async def readexactly(self, n):
                if n == 2:
                    return b'  '
                return b'\x00' * large_length
    ...
    @pytest.mark.asyncio
    async def test_http_read_packet_incomplete_line(self, mock_loggers):
        """Test reading HTTP packet with incomplete line."""
        conn = ConnectionHttp('127.0.0.1', 443, 1, loggers=mock_loggers)
        codec = HttpPacketCodec(conn)

        reader_mock = MagicMock()
        reader_mock.readline = AsyncMock(return_value=b'incomplete')

        with pytest.raises(asyncio.IncompleteReadError):
            await codec.read_packet(reader_mock)

    @pytest.mark.asyncio
    async def test_http_read_packet_multiple_headers(self, mock_loggers):
        """Test reading HTTP packet with multiple headers."""
        conn = ConnectionHttp('127.0.0.1', 8080, 1, loggers=mock_loggers)
        codec = HttpPacketCodec(conn)

        # Simulate HTTP response with multiple headers before content-length
        class MockReader:
            def __init__(self):
                self.call_count = 0
...
class TestObfuscatedConnection:
    """Tests for ObfuscatedConnection class."""

    def test_obfuscated_init(self, mock_loggers):
        """Test Obfuscated connection initialization."""
        conn = ConnectionTcpObfuscated('127.0.0.1', 443, 1, loggers=mock_loggers)

        assert conn.obfuscated_io == ObfuscatedIO
        assert conn.packet_codec == AbridgedPacketCodec

    @pytest.mark.asyncio
    async def test_obfuscated_connection_init_conn(self, mock_loggers, mock_writer):
        """Test ObfuscatedConnection _init_conn."""
        conn = ConnectionTcpObfuscated('127.0.0.1', 443, 1, loggers=mock_loggers)
        conn._writer = mock_writer

        # Call _init_conn - it should call ObfuscatedIO.__init__ and write header
        conn._init_conn()
    ...
    @pytest.mark.asyncio
    async def test_obfuscated_connection_send(self, mock_loggers):
        """Test ObfuscatedConnection _send."""
        conn = ConnectionTcpObfuscated('127.0.0.1', 443, 1, loggers=mock_loggers)
        # Initialize codec since _connect hasn't been called
        from telethon.network.connection.tcpabridged import AbridgedPacketCodec
        conn._codec = AbridgedPacketCodec(conn)

        # Create mock obfuscation that tracks encryption
        encrypt_mock = MagicMock(return_value=b'encrypted')
        decrypt_mock = MagicMock(return_value=b'decrypted')
        conn._obfuscation = MagicMock()
        conn._obfuscation._encrypt = encrypt_mock
        conn._obfuscation._decrypt = decrypt_mock
        conn._writer = MagicMock()
        conn._writer.write = MagicMock()

        # Call send - should encode packet, encrypt, and write
        conn._send(b'test data')

        # Verify: encrypted data was written
        conn._obfuscation._encrypt.assert_called_once()
        conn._writer.write.assert_called_once_with(b'encrypted')

    @pytest.mark.asyncio
    async def test_obfuscated_connection_recv(self, mock_loggers):
        """Test ObfuscatedConnection _recv."""
        conn = ConnectionTcpObfuscated('127.0.0.1', 443, 1, loggers=mock_loggers)
        # Initialize codec since _connect hasn't been called
        from telethon.network.connection.tcpabridged import AbridgedPacketCodec
        conn._codec = AbridgedPacketCodec(conn)

        # Create mock obfuscation that tracks decryption
        decrypt_mock = MagicMock(return_value=b'result')
        conn._obfuscation = MagicMock()
        conn._obfuscation._decrypt = decrypt_mock
        conn._codec.read_packet = AsyncMock(return_value=b'test')

        # Call recv - should read packet and decrypt
        result = await conn._recv()

        # Verify: data was decrypted
        conn._obfuscation._decrypt.assert_called_once()
